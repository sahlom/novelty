...
@section('title', 'Estatus - ' . $status->name)
...
@section('content_header')
    <div class="d-flex justify-content-between align-items-center w-100 flex-wrap">
        ...
        <div class="mt-2 mt-md-0">
            ...
            <a href="{{ route('statuses.edit', $status->id) }}" class="btn btn-primary btn-sm shadow-sm mr-2">
            ...
            @if(auth()->user()->role === 'admin')
                @if($status->tasks()->exists())
                    <button class="btn btn-primary btn-sm shadow-sm mr-2" style="opacity: 0.6; cursor: not-allowed;" 
                            title="No se puede eliminar: Este Estatus cuenta con tareas registradas en el sistema." disabled>
                        <i class="fas fa-ban"></i> Eliminar
                    </button>
                @else
                    <form action="{{ route('statuses.destroy', $status->id) }}" method="POST" class="d-inline" onsubmit="return confirm('¿Estás absolutamente seguro de eliminar este Estatus? Esta acción borrará el registro del catálogo de forma irreversible.')">
                        @csrf 
                        @method('DELETE')
            ...
            <div class="card-body">
                <div class="d-flex align-items-center justify-content-between">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            <h3 class="font-weight-bold text-dark mb-1">{{ $status->name }}</h3>
                        </div>
                    </div>
                    
                    <div>
                        {{-- <span><strong>Total Tareas:</strong> {{ $status->tasks?->count() ?? 0 }}</span> --}}
                        <h3 class="font-weight-bold text-dark mb-1">Total Tareas: {{ $status->tasks?->count() ?? 0 }}</h3>
                    </div>
                </div>
            </div>
        </div>
